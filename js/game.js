...
function Peep(config, level) {
  ...
  self.sway = 0;
  var swayVel = 0;
  var bouncy = [0.0, 0.25, 1.0, 0.9, 0.0, 0.0, 0.25, 1.0, 0.9, 0.0];
  self.draw = function (ctx) {
    var x = self.x;
    var y = self.y;
    ...
    function update() {
      ...
      if (STAGE == 3 && !window.HAS_PLAYED_JAZZ) {
        if (STAGE == 3 && CURRENT_LEVEL == 1) {
          var framesLeft =
            rewindLevel.frames.length -
            rewindFrame +
            levelObjects[2].frames.length;
          if (framesLeft < 135) {
...
function next() {
  ...
  if (CURRENT_LEVEL < LEVEL_CONFIG.length) {
    ...
    var totalFrames =
      levelObjects[0].frames.length +
      levelObjects[1].frames.length +
      levelObjects[2].frames.length;
...
function iHeartYou() {
  ...
  document.getElementById("canvas_container").style.backgroundPosition =
    "0px -390px";
  ...
  if (window.location.hash) {
    vtext.textContent = encryptString(
      decodeURIComponent(window.location.hash).substring(1)
    );
  } else {
    vtext.textContent = "I love you more and more everyday <3";
  }
...
//     case "tumblr":
//       url +=
//         "https://www.tumblr.com/share/link?url=" + encodeURIComponent(link);
...
//     case "reddit":
//       window.open(
//         "http://www.reddit.com/submit?v=5&amp;noui&amp;jump=close&amp;url=" +
//           encodeURIComponent(link) +
//           "&amp;title=" +
//           encodeURIComponent("it's a(door)able: a one-minute minigame"),
//         "reddit",
//         "toolbar=no,width=700,height=550"
//       );
//       return false;
//       break;
//     case "stumbleupon":
//       url +=
//         "http://www.stumbleupon.com/submit?url=" + encodeURIComponent(link);
//       break;
//   }
